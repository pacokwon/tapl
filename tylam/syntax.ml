...
let rec typecheck_helper ctx = function
  ...
  | TypedCond (cond, t, f) -> (
      ...
      | Bool ->
          ...
          if tt = ft then tt else failwith "Not")
  | TypedVar v -> (
  ...
  | TypedApp (t1, t2) -> (
      ...
      | Arrow (tp1, tp2) ->
          if tp1 = typecheck_helper ctx t2 then tp2
          else
            failwith
              "RHS of a function application must match the input type of the \
               LHS")
...
let to_debruijn t =
  ...
  let rec helper ctx depth = function
    ...
    | TmVar n ->
        let index =
          match Context.find_opt n ctx with
          | Some i -> i
          | None -> failwith "Variable name not found in context."
        in
        DeVar (depth - 1 - index)
    | TmAbs (name, term) ->
...
let rec d_place_shift d cutoff = function
  ...
  | DeCond (cond, t, f) ->
      DeCond
        ( d_place_shift d cutoff cond,
          d_place_shift d cutoff t,
          d_place_shift d cutoff f )
  | DeVar k as v -> if k < cutoff then v else DeVar (k + d)
  | DeAbs t -> DeAbs (d_place_shift d (cutoff + 1) t)
  | DeApp (t1, t2) ->
      DeApp (d_place_shift d cutoff t1, d_place_shift d cutoff t2)

let is_value = function DeAbs _ -> true | _ -> false

let substitute_top body argument =
  let rec helper depth body argument =
    match body with
    | DeTrue -> DeTrue
    | DeFalse -> DeFalse
    | DeCond (cond, t, f) ->
        DeCond
          ( helper depth cond argument,
            helper depth t argument,
            helper depth f argument )
    | DeVar index as v -> if depth = index then argument else v
    | DeAbs term -> DeAbs (helper (depth + 1) term (d_place_shift 1 0 argument))
    | DeApp (t1, t2) ->
